...
%   Created:  16 May 2020   from mrGEDI_OutdcGC.m (KY) 
...
if ~(isfield(GCparam,'DynHPAF') == 1 && strncmp(GCparam.DynHPAF.StrPrc,'frame',5) == 1),
    % normal processing  using sample-by-sample GCFB
    SwGCFBframebase = 0;
    ...
    SwGCFBframebase = 1;
    fsEnv = GCparam.DynHPAF.fs;  % fs of frame-base GCFB
end;
...
% Frame processings
if SwGCFBframebase == 0
    lenAudModEnvs = ceil(length(OutdcGCClean)*(fsEnv/fsdcGC));
else
    [~, lenAudModEnvs] = size(OutdcGCClean);
end;
...
if find(lenWin == lenAudModEnvs) % If the duration of the stimulus is exactly equalt to the window duration
    ...
    numSegWin(segIdx) =  max(numSegWin(segIdx)-1,1);   % To avoid zero division, 26 May 20, IT
end
...
for nAud = numChAud % 1:GCparam.NumCh
    ...
    else % frame-base processing
        EnvMix      = filter(bzLPF,apLPF,OutdcGCMix(nAud,:));    % LPF
        EnvClean  = filter(bzLPF,apLPF,OutdcGCClean(nAud,:)); % LPF
        %   disp('SwGCFBframebase==1');
    end;
